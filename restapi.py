...
def predict(model):
    ...
    if request.data:
        ...
        if model in models:
            ...
            results = results.render()[0]  # 用于将检测结果渲染到图像上
            return cv2.imencode(".jpg", results)[1].tobytes()  # 编码为二进制返回
...
if __name__ == "__main__":
    ...
    for m in opt.model:
        models[m] = torch.hub.load("./", m, source='local', path="../yolov5-7.0/runs/train/exp3/weights/best.pt")
